      ...
      <section className="glass card p-6 sm:p-8">
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-2">
          <div className="rounded-3xl border border-white/[0.07] bg-black/10 p-5"><h2 className="font-bold">ترکیب دارایی</h2><div className="mt-4 space-y-3">{[["سهام",fund.stockPct],["اوراق",fund.bondPct],["سپرده",fund.depositPct],["کالا",fund.commodityPct],["سایر",fund.otherPct]].map(([label,value]) => <div key={String(label)}><div className="mb-1.5 flex justify-between text-xs"><span className="text-white/55">{label}</span><span>{percent(value as number | null,1)}</span></div><div className="h-2 overflow-hidden rounded-full bg-white/[0.05]"><div className="h-full rounded-full bg-lime-200/70" style={{ width: `${Math.max(0, Math.min(100, Number(value || 0)))}%` }}/></div></div>)}</div></div>
            <div className="rounded-3xl border border-white/[0.07] bg-black/10 p-5"><h2 className="font-bold">مشخصات داده</h2><dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-4 text-xs"><div><dt className="text-white/35">شناسه منبع</dt><dd className="mt-1 break-all">{fund.regNo}</dd></div><div><dt className="text-white/35">InsCode</dt><dd className="mt-1">{fund.insCode || "—"}</dd></div><div><dt className="text-white/35">نوع سرمایه‌گذاری</dt><dd className="mt-1">{fund.typeOfInvest || "—"}</dd></div><div><dt className="text-white/35">نوع صندوق</dt><dd className="mt-1">{fund.fundTypeName || "—"}</dd></div><div><dt className="text-white/35">صدور امروز</dt><dd className="mt-1">{faNumber(fund.unitsSubDay)}</dd></div><div><dt className="text-white/35">ابطال امروز</dt><dd className="mt-1">{faNumber(fund.unitsRedDay)}</dd></div></dl></div>
        </div>
